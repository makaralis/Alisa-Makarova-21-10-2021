...
export const setMyCityData = async (geoLocation, setIsCitySelected, setChosenCity, setCityKey, setForecast, setCurrentForecast) => {
    try {
        ...
        if (data) {
            ...
            await getForecast(data.ParentCity?.Key, (val) => setForecast(val), (val) => setCurrentForecast(val));
        }
    }
    catch (err) {
